...
//    double b = 0.0;
//    for (; y > 0; y--) {
//      final int blockAndMeta = BlockColourGen.calculateBlockAndMeta(chunk.getBlock(x, y, z), chunk.getBlockMetadata(x, y, z));
//
//      int c1 = bc.getColour(blockAndMeta);
//      int alpha = (c1 >> 24) & 0xff;
//      // no need to process block if it is transparent
//      if (alpha > 0) {
//        int biome = chunk.get(x, z);
...
//  }
//
//  static int getChunkHeight(final Chunk chunk) {
//    final ExtendedBlockStorage[] storageArray = chunk.getBlockStorageArray();
//    int result = 0;
//    while (storageArray[result] != null) {
//      ++result;
//    }
//    --result;
//    result <<= 4;
...
//  }
//
//  public static void renderSurface(final BlockColours bc, final Chunk chunk, final int[] pixels, final int offset, final int scanSize, final boolean dimensionHasCeiling) {
//    int y;
//    for (int z = 0; z < MwChunk.SIZE; z++) {
//      for (int x = 0; x < MwChunk.SIZE; x++) {
//        y = chunk.getHeightValue(x, z);
